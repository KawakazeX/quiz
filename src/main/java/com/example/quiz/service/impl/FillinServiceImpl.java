...
public class FillinServiceImpl implements FillinService {
    ...
    public BasicRes fillin(FillinReq req) {
        ...
        if (responseDao.existsByQuizIdAndPhone(req.getQuizId(), req.getPhone())) {
            return new BasicRes(ResponseMessage.DUPLICATE_FILLIN.getCode(), //
                    ResponseMessage.DUPLICATE_FILLIN.getMessage());
        }
        // 檢查 quiz_id 是否存在於DB中
        // 因為後續會比對 req 中的答案與題目的選項是否符合，所以要用 findById
        Optional<Quiz> op = quizDao.findById(req.getQuizId());
        if (op.isEmpty()) {
            return new BasicRes(ResponseMessage.QUIZ_NOT_FOUND.getCode(), ResponseMessage.QUIZ_NOT_FOUND.getMessage());
        }
        Quiz quiz = op.get();
        ...
        try {
            ...
            for (Question item : quList) { // 1(必) 2(必) 3(選)
                ...
                // 正常的情況是: 問題是必填，然後又有回答，每跑完一 題，qIds 就會包含該必填問題的 id
                // 因此若問題是必填但 qIds 又沒有包含該題的 id，就表示沒有回答該題
                if (item.isNecessary() && !qIds.contains(item.getId())) {
                    return new BasicRes(ResponseMessage.ANSWER_IS_REQUIRED.getCode(),
                            ResponseMessage.ANSWER_IS_REQUIRED.getMessage());
                }
            }
    ...
    public FeedbackRes feedback(FeedbackReq req) {
        ...
        Optional<Quiz> op = quizDao.findById(req.getQuizId());
        if (op.isEmpty()) {
            return new FeedbackRes(ResponseMessage.QUIZ_NOT_FOUND.getCode(),
                    ResponseMessage.QUIZ_NOT_FOUND.getMessage());
        }
        // 取得 quiz
        Quiz quiz = op.get();
        ...
        try {
            ...
            // 遍歷 resList
            for (Response resItem : responseList) {
                ...
                FeedbackDetail detail = new FeedbackDetail(quiz.getName(), quiz.getDescription(),
                        quiz.getStartDate(), quiz.getEndDate(), resItem.getName(),
                        resItem.getPhone(), resItem.getEmail(), resItem.getAge(), fillinList);
                Feedback feedback = new Feedback(resItem.getName(), resItem.getFillinDateTime(), detail);
                feedbackList.add(feedback);

            }
        } catch (Exception e) {
            return new FeedbackRes(ResponseMessage.JSON_PROCESSING_EXCEPTION.getCode(),
                    ResponseMessage.JSON_PROCESSING_EXCEPTION.getMessage());
        }
        return new FeedbackRes(ResponseMessage.SUCCESS.getCode(),
                ResponseMessage.SUCCESS.getMessage(), feedbackList);
    }


    private BasicRes checkParams(FillinReq req) {
        if (req.getQuizId() <= 0) {
            return new BasicRes(ResponseMessage.PARAM_QUIZ_ID_ERROR.getCode(),
                    ResponseMessage.PARAM_QUIZ_ID_ERROR.getMessage());
        }
        if (!StringUtils.hasText(req.getName())) {
            return new BasicRes(ResponseMessage.PARAM_NAME_IS_REQUIRED.getCode(),
                    ResponseMessage.PARAM_NAME_IS_REQUIRED.getMessage());
        }
        if (!StringUtils.hasText(req.getPhone())) {
            return new BasicRes(ResponseMessage.PARAM_PHONE_IS_REQUIRED.getCode(),
                    ResponseMessage.PARAM_PHONE_IS_REQUIRED.getMessage());
        }
        if (!StringUtils.hasText(req.getEmail())) {
            return new BasicRes(ResponseMessage.PARAM_EMAIL_IS_REQUIRED.getCode(),
                    ResponseMessage.PARAM_EMAIL_IS_REQUIRED.getMessage());
        }
        if (req.getAge() <= 12 || req.getAge() > 99) {
            return new BasicRes(ResponseMessage.PARAM_AGE_NOT_QUALIFIED.getCode(),
                    ResponseMessage.PARAM_AGE_NOT_QUALIFIED.getMessage());
        }
    ...
    private BasicRes checkOptionAnswer(Question item, Fillin fillin) {
        ...
        if (item.getType().equalsIgnoreCase(OptionType.SINGLE_CHOICE.getType()) //
            ...
            return new BasicRes(ResponseMessage.ANSWER_OPTION_TYPE_IS_NOT_MATCH.getCode(),
                    ResponseMessage.ANSWER_OPTION_TYPE_IS_NOT_MATCH.getMessage());
        }
    ...
    public StatisticsRes statistics(FeedbackReq req) {
        ...
        for (Response item : responseList) {
            ...
            try {
                ...
                return new StatisticsRes(ResponseMessage.JSON_PROCESSING_EXCEPTION.getCode(),
                        ResponseMessage.JSON_PROCESSING_EXCEPTION.getMessage());
            }
        }
        Optional<Quiz> op = quizDao.findById(req.getQuizId());
        if(op.isEmpty()) {
            return new StatisticsRes(ResponseMessage.QUIZ_NOT_FOUND.getCode(),
                    ResponseMessage.QUIZ_NOT_FOUND.getMessage());
        }
        Quiz quiz = op.get();
        return new StatisticsRes(ResponseMessage.SUCCESS.getCode(), ResponseMessage.SUCCESS.getMessage(), //
